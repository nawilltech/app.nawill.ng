...
> This table reflects the actual build in this repository, not just the target design. A few items from the original Phase 1 scope were deliberately trimmed for this pass — milestones/change-history/status-request sub-resources, blog/FAQ, and file upload handling — in favor of going deep on the modules that carry money or access-control risk (auth, KYC, projects, invoices, support, payments, wallet). Email verification, originally trimmed alongside these, was added later in this build (`POST /auth/verify-email`/`resend-verification`, real Gmail delivery via `MailService` — see [Architecture doc §7.7](./ARCHITECTURE.md#77-redis-backed-auth-security)). Nothing designed in the [Architecture doc](./ARCHITECTURE.md) was removed; these are sequencing choices, not scope cuts — see [QA.md §7](./QA.md#7-known-gaps-explicitly-out-of-scope-for-this-pass) for the full list.
